...
class API extends REST
{
    ...
    public function check(){
        ...
        $user = mysqli_query($this->mysqli, "SELECT * FROM `user`");
        $row = $user->fetch_assoc();print_R($row);
        $user_rlt = mysqli_query($this->mysqli, "INSERT INTO `user` (uid, full_name, email, mobile, password, salt,otp,is_verified)
                                                            VALUES('1234', '$full_name', '$email', '$mobile', '$password', '$salt','$otp','$is_verified')");
    ...
    public function check1(){
        $user_rlt = mysqli_query($this->mysqli, "INSERT INTO `user` (uid, full_name, email, mobile, password, salt,otp,is_verified)
        VALUES('dsfaya234', 'beema', 'beema@gmail.com', '9896768559', 'beema@123', 'rsfgsh',1234,1)");
        echo mysqli_num_rows($user_rlt);
        // $success = array('status' => "success", 'msg' =>$user_rlt);
        //     $this->response($this->json($success), 200);
     ...
     public function otpVerification()
    {
        ...
        if (mysqli_num_rows($user)) {
            ...
            if ($user_rlt) {
                    // Return success with generated OTP
                    $success = array('status' => "Success", 'msg' => "Otp verified", 'uid' => $uid,'is_verified' => true,'user' => $user_data);
                    $this->response($this->json($success), 200);
            } else {
...
public function resendOtp()
    {
        ...
        $uid = isset($this->_request['uid']) ? mysqli_real_escape_string($this->mysqli, $this->_request['uid']) : null;
        $usermobile= mysqli_query($this->mysqli, "SELECT mobile
                                                    FROM `user`
                                                    WHERE (uid = '$uid')");
                                             
        if (mysqli_num_rows($usermobile)) {
            $row = $usermobile->fetch_assoc();
            $mobile=$row['mobile'];
        }
        $otpstatus = $this->db_user->sendOtp($mobile);
